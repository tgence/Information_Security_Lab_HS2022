...
def recover_x_known_nonce(k, h, r, s, q):
    # All the arguments are of type int
...
def recover_x_repeated_nonce(h_1, r_1, s_1, h_2, r_2, s_2, q):
    ...
    # The function should compute and return the secret signing key x
    signatures_term_inv = mod_inv(r_2 * s_1 - r_1 * s_2 , q)
    x = (h_1 * s_2 - h_2 * s_1) * signatures_term_inv % q
    return x
...
def MSB_to_Padded_Int(N, L, list_k_MSB):
    ...
    list_k_MSB_to_string = ''.join(str(bit) for bit in list_k_MSB)
    a = int(list_k_MSB_to_string[0:L], 2)
...
def LSB_to_Int(list_k_LSB):
    ...
    return int(''.join(str(bit) for bit in list_k_LSB), 2)
...
def setup_hnp_single_sample(N, L, list_k_MSB, h, r, s, q, givenbits="msbs", algorithm="ecdsa"):
    ...
    if algorithm == "ecdsa" and givenbits == "msbs":
        t = (r * mod_inv(s, q)) % q                                                     # rs^-1 mod q
        u = (MSB_to_Padded_Int(N, L, list_k_MSB) - (h * mod_inv(s, q))) % q             # k - z mod q where z = hs^-1 
    
    elif algorithm == "ecdsa" and givenbits == "lsbs":  
        t = ((r * mod_inv(s, q)) * mod_inv(2**L, q)) % q                                # rs^-1 * 2^-L mod q
        u = ((LSB_to_Int(list_k_MSB) - (h * mod_inv(s, q))) * mod_inv(2**L, q)) % q     # (k-z) * 2^-Lmod q where z = hs^-1   
   
    elif algorithm == "ecschnorr" and givenbits == "msbs":
        t = h                                                                           # From sheet, we have (h, s) pairs
        u = (MSB_to_Padded_Int(N, L, list_k_MSB) - s) % q                               # From s = k − h * x mod q => u = k - s

    else:
        t = (h* mod_inv(2**L, q)) % q
        u = ((LSB_to_Int(list_k_MSB) - s) * mod_inv(2**L, q)) % q
...
def hnp_to_cvp(N, L, num_Samples, list_t, list_u, q):
    ...
    list_u.append(0)
    list_u = [u * 2**(L+1) for u in list_u]
    
    B_CVP = []
    for i in range(num_Samples + 1):
        row = []
        for j in range(num_Samples + 1):
            if(i == j and j != num_Samples):
                row.append(q)
            elif(i == num_Samples):
                if(j == num_Samples):
                    row.append(1 / 2**(L+1))
                ...
                row.append(0)
        B_CVP.append([int(element * 2**(L+1)) for element in row])
...
def cvp_to_svp(N, L, num_Samples, cvp_basis_B, cvp_list_u):
    ...
    for i in range(num_Samples + 1):
        row = []
        for j in range(num_Samples + 1):
            row.append(cvp_basis_B[i][j])
        row.append(0)
        B_SVP.append(row)

    row = []
    for i in range(num_Samples + 1):
        row.append(cvp_list_u[i])
    row.append(M)
    B_SVP.append(row)
...
def solve_cvp(cvp_basis_B, cvp_list_u):
    ...
    return list(CVP.closest_vector(cvp_basis_B, cvp_list_u))
...
def recover_x_partial_nonce_SVP(Q, N, L, num_Samples, listoflists_k_MSB, list_h, list_r, list_s, q, givenbits="msbs", algorithm="ecdsa"):
    ...
    i = 0
    v = (cvp_list_u[0] - list_of_f_List[1][0]) % q

    while(check_x(v, Q) == False and i < len(cvp_list_u)):

        v = (cvp_list_u[i] - list_of_f_List[1][i]) % q
        x = v % q
        i += 1
